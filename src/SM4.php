...
class SM4
{
    ...
    public static function sm4Decrypt($cipherText)
    {
        $x = [];
        for ($j = 0; $j < 4; $j++) {
        ...
        return self::bytesToString($plainText);
    }

    public static function sm4Encrypt($plainText)
    {
        $x = [];
        for ($j = 0; $j < 4; $j++) {
    ...
    public static function sm4Rotl32($buf, $n)
    {
        return (($buf << $n) & 0xffffffff) | ($buf >> (32 - $n));
    }

    public static function sm4KeySchedule($key)
    {
        self::$_rk = [];
     ...
     * @return int
     */
    public static function getI(int $tmp): int
    {
